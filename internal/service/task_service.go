...
func NewTaskService(taskRepo ports.TaskRepository, noteRepo ports.NoteRepository, eventPublisher ports.EventPublisher, logger *slog.Logger, cache ports.CacheRepository) *TaskService {
	return &TaskService{
		taskRepo: taskRepo,
		noteRepo: noteRepo,
...
func (s *TaskService) Create(ctx context.Context, userID string, title string, description string, deadline *time.Time) (*domain.Task, error) {
	...
	s.cache.Delete(ctx, "tasks:"+userID)
